...
// ✅ [로그인 체크 함수 강화]
// 문자열 "null", "undefined", 공백 등을 확실하게 체크합니다.
function isUserLoggedIn(id) {
  if (!id) return false; // null, undefined, ""
...
async function getShipmentDetail(oid) {
  const res = await apiRequest("GET", `https://${CAFE24_MALLID}.cafe24api.com/api/v2/admin/orders/${oid}/shipments`, {}, { shop_no: 1 });
  if (res.shipments?.[0]) {
    ...
    if (isUserLoggedIn(memberId)) {
      try {
        ...
        if (data.orders?.[0]) {
          ...
          return s ? { text: `최근 주문(${t.order_id})은 ${s.shipping_company_name} 배송 중입니다.` } : { text: "최근 주문 확인 중입니다." };
        }
        return { text: "최근 2주 내 주문 내역이 없습니다." };
      } catch (e) { return { text: "조회 실패." }; }
    } else {
      // 🚨 여기가 요청하신 로그인 버튼 뜨는 부분입니다.
