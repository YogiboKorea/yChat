;(function () {
    ...
    async function fetchProducts(directNosAttr, category, limit = 300) {
        ...
        if (directNosAttr) {
            ...
            const results = await Promise.all(ids.map(no => fetchWithRetry(`${API_BASE}/api/${mallId}/products/${no}${couponQSStart}`, fetchOpts).then(r => r.json())));
            ...
            const prods = await fetchWithRetry(prodUrl, fetchOpts).then(r => r.json()).then(json => Array.isArray(json) ? json : (json.products || []));
            return prods;
        }
        return [];
    }
              ...
              <li style="overflow: hidden; border: 1px solid #e8e8e8; background: #fff;">
                <a href="/product/detail.html?product_no=${p.product_no}" style="text-decoration:none; color:inherit;">
                  ...
                  <div style="padding: 12px; min-height: 90px;">
                    <div style="font-weight: 500; font-size: ${titleFontSize}; line-height: 1.2;">${escapeHtml(p.product_name || '')}</div>
                    ${p.price != null ? `<div style="font-weight: bold; font-size: ${priceFontSize}; margin-top: 4px;">${formatKRW(p.price)}</div>` : ''}
                  </div>
                </a>
              </li>`;
        }).join('');
