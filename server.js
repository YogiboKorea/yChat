...
//카페24 에서 주문 페이지 금액및 이벤트 참여관련 데이터 유지용 코드 추가하기 
...
app.listen(PORT, () => {
    ...
    console.log(`매출액 데이터는 http://localhost:${PORT}/api/sales 에서 확인할 수 있습니다.`);

    // 서버가 시작되면 먼저 한 번 매출액을 계산하고,
    calculateSalesForPeriod(TARGET_START_DATE, TARGET_END_DATE);

    // 그 후 3분마다 주기적으로 업데이트합니다.
    setInterval(() => {
        calculateSalesForPeriod(TARGET_START_DATE, TARGET_END_DATE);
    }, 3 * 60 * 1000);
});
