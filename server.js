...
app.get('/api/:_any/eventTemple/:id', async (req, res) => {
  ...
  await runDb(async (db) => { const ev = await db.collection(EVENT_COLL).findOne({ _id: new ObjectId(req.params.id), mallId: MALL_ID }); res.json(ev); });
});
app.post('/api/:_any/eventTemple', async (req, res) => {
  const doc = { ...req.body, createdAt: new Date() }; if(doc.content?.blocks) doc.content.blocks = normalizeBlocks(doc.content.blocks);
...
app.get('/api/:_any/categories/:category_no/products', async (req, res) => {
  ...
  try {
    ...
    // 2. 상품 상세 정보 조회
    const detailRes = await apiRequest('GET', `https://${MALL_ID}.cafe24api.com/api/v2/admin/products`, {}, { shop_no: 1, product_no: productNos.join(','), limit: productNos.length, fields: 'product_no,product_name,price,list_image,summary_description,icons' });
    ...
    await Promise.all(productNos.map(async no => {
      try {
        const d = await apiRequest('GET', `https://${MALL_ID}.cafe24api.com/api/v2/admin/products/${no}/discountprice`, {}, { shop_no: 1 });
        discountMap[no] = d.discountprice?.pc_discount_price || null;
      } catch (e) { discountMap[no] = null; }
    }));

    // 응답 조립
    const result = details.map(p => ({
...
app.get('/api/:_any/products/:product_no', async (req, res) => {
  ...
  try {
    ...
    // 아이콘/할인 정보 등 추가 로직은 간소화하여 제공 (필요 시 확장 가능)
...
app.get('/api/:_any/analytics/:pageId/visitors-by-date', async (req, res) => {
  ...
  await runDb(async (db) => {
    const stats = await db.collection(`visits_${MALL_ID}`).aggregate([
      { $match: match },
      { $group: { _id: { date: '$dateKey' }, viewCount: { $sum: '$viewCount' }, revisitCount: { $sum: '$revisitCount' } } },
      { $sort: { _id: 1 } }
    ]).toArray();
...
app.get('/api/:_any/analytics/:pageId/clicks-by-date', async (req, res) => {
  ...
  await runDb(async (db) => {
    const data = await db.collection(`clicks_${MALL_ID}`).aggregate([
      { $match: match },
      { $group: { _id: { date: '$dateKey' }, count: { $sum: 1 } } },
      { $sort: { _id: 1 } }
    ]).toArray();
